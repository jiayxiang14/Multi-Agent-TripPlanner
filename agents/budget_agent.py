...
# 各消费等级目的地的最低可行预算（人均，含机票+酒店+活动）
# 生产环境应从真实数据动态计算，这里用经验值作为硬性门槛
MIN_VIABLE_BUDGET: dict[str, float] = {
    "low": 4000,     # 东南亚：曼谷、清迈等
    "medium": 8000,  # 东北亚：首尔、大阪等
    "high": 15000,   # 欧美日：东京、巴黎、伦敦等
}
...
class BudgetAgent(BaseAgent):
    ...
    async def execute(self, state: TravelPlanState) -> TravelPlanState:
        ...
        # ── 前置校验：预算是否根本不可行 ──────────────────────
        # 在进入搜索和调整循环之前就拦截，避免生成无意义的假方案
        dest = state.selected_destination
        if dest is not None:
            per_person_budget = pref.budget / max(pref.num_travelers, 1)
            min_budget = MIN_VIABLE_BUDGET.get(dest.cost_level, 8000)

            if per_person_budget < min_budget * 0.5:
                # 人均预算连最低门槛的一半都不到，直接判定不可行
                msg = (
                    f"预算不足：{dest.city}（{dest.country}）人均最低需要 ¥{min_budget}，"
                    f"当前人均预算仅 ¥{per_person_budget:.0f}。"
                    f"建议提高预算至 ¥{min_budget * pref.num_travelers:.0f} 以上，"
                    f"或选择消费更低的目的地（如东南亚）。"
                )
                state.error_messages.append(msg)
                state.state = PlanningState.FAILED
                logger.warning(f"[{self.name}] 预算不可行: {msg}")
                return state
